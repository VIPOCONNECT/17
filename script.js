...
function generatePreview() {
  ...
  // ניקוי התצוגה המקדימה
  preview.open();
  
  // יצירת ה-HTML לתצוגה מקדימה
  const html = generateHTML();
  
  // הכנסת ה-HTML לתצוגה המקדימה
  preview.write(html);
  preview.close();
}

function downloadHTML() {
  // עדכון התצוגה המקדימה לפני הורדה
  generatePreview();
  
  // קבלת ה-HTML מה-iframe
  const previewFrame = document.getElementById('previewFrame');
  const html = previewFrame.contentWindow.document.documentElement.outerHTML;
